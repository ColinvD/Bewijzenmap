...
public class Template extends Application{  //zorg dat de class overerft van Application
  public void start(Stage stage){   //geef de stage mee als argument
    ...
    //inladen van een Image in een ImageView
        ...
        Circle rondje = new Circle();
        rondje.setCenterX(100.0f);
        rondje.setCenterY(100.0f);
        rondje.setRadius(125.0f);
        Paint paint = Color.BLUE;
        rondje.setFill(paint);
        rondje.relocate(0,0);
        group2.getChildren().add(rondje);
        ...
        ParallelTransition transition = new ParallelTransition(rondje,translate, fill, rotate);
    ...
    TextField tekstveld = new TextField("ik ben een tekstveld");
    tekstveld.relocate(50,80);
    group1.getChildren().add(tekstveld);

    ProgressBar voortgang = new ProgressBar();
    voortgang.relocate(50,110);
    rootNode.getChildren().add(voortgang);
    ...
    //nadat je het omgedraaid gaat de button achter het plaatje
    //dat komt omdat de button eerst word gerenderd en de group eroverheen
    //
