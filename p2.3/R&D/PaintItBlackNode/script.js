...
socket.on('change', function(picture){
  console.log(picture);
  for (i = 0; i < 64; i++) {
    drawing[i].x = picture[i].x;
    drawing[i].y = picture[i].y;
    drawing[i].color = picture[i].color;
...
init();
socket.emit('start');
socket.on('start', function(picture){
  for (i = 0; i < 64; i++) {
    drawing[i].x = picture[i].x;
    drawing[i].y = picture[i].y;
    drawing[i].color = picture[i].color;
